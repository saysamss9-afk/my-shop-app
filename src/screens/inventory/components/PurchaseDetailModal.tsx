            ...
            <VStack space="lg" py="$4">
                    ...
                    <Box key={idx} py="$2">
                        <HStack justifyContent="space-between" alignItems="center">
                            <VStack flex={1}>
                                ...
                                <Text size="xs" color="$text500">{item.quantity} {item.isBulk ? 'Cartons' : 'Units'} @ ₵{item.costPrice.toFixed(2)}</Text>
                            </VStack>
                            <HStack space="md" alignItems="center">
                                <Text size="sm" fontWeight="$black">₵{(item.quantity * item.costPrice).toFixed(2)}</Text>
              ...
              <Box borderTopWidth={1} borderColor="$borderLight" pt="$4">
                <HStack justifyContent="space-between" mb="$1">
                    ...
                    <Text size="sm" fontWeight="$bold">₵{purchase.totalCost.toFixed(2)}</Text>
                </HStack>
                <HStack justifyContent="space-between" mb="$1">
                    <Text size="sm" color="$success700">Amount Paid</Text>
                    <Text size="sm" fontWeight="$bold" color="$success700">- ₵{purchase.amountPaid.toFixed(2)}</Text>
                </HStack>
                <Divider my="$2" />
                <HStack justifyContent="space-between">
                    <Heading size="sm">Balance Owed</Heading>
                    <Heading size="sm" color="$error600">₵{purchase.balance.toFixed(2)}</Heading>
                </HStack>
              </Box>
            </VStack>
          </ScrollView>
